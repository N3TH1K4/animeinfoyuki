...
**ID | MAL ID:** `{idm}` | `{idmal}`
♻️ **SOURCE:** `{source}`
♻️ **TYPE:** `{formats}`{avscd}{dura}{user_data}
♻️ **ADULT RATED:** `{adult}`
...
async def get_anime(vars_, auth: bool = False, user: int = None):
    ...
    avscd = f"\n⚜️ **SCORE:** `{score}%` ❤️‍🔥" if score is not None else ""
    tags = []
    for i in data['tags']:
        tags.append(i["name"])
...
async def get_manga(qdb, page, auth: bool = False, user: int = None):
    ...
    finals_ = f"{name}\n\n"
    finals_ += f"♣️ **ID:** `{idm}`\n"
    finals_ += f"♥️ **STATUS:** `{status}`\n"
    finals_ += f"♠️ **VOLUMES:** `{volumes}`\n"
    finals_ += f"♦️ **CHAPTERS:** `{chapters}`\n"
    finals_ += f"⚫️ **SCORE:** `{score}`\n"
    finals_ += f"⚪️ **FORMAT:** `{format_}`\n"
